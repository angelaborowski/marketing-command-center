/**
 * Server-Side Writer Agent
 *
 * Same logic as src/lib/agents/implementations/writerAgent.ts but
 * imports from server-side shims to avoid browser dependencies.
 */

import {
  WRITER_SYSTEM_PROMPT,
  buildWriterUserPrompt,
} from '../src/lib/agents/prompts/writerPrompt';
import { callAgentClaude } from './agentClaude.server';
import { validateContent } from './validate.server';
import type {
  AgentDefinition,
  AgentContext,
  AgentCallbacks,
...
import type { ContentItem, ContentType, Pillar, Platform, ExamLevel } from '../src/types';
import { PLATFORM_CONTENT_TYPES } from '../src/types';
...
export const serverWriterAgent: AgentDefinition<WriterInput, WriterOutput> = {
  id: 'writer',
  name: 'Mildred',
  description:
    'Generates viral educational content items using AI, guided by gap analysis. (Server-side)',
  icon: 'pen-tool',
  color: 'text-purple-500',

  canRun(_context: AgentContext) {
    if (!process.env.CLAUDE_API_KEY) {
      return { ok: false, reason: 'CLAUDE_API_KEY not set in server environment.' };
    }
    return { ok: true };
  },
